...
Results_directory_name = paste0("HS_Results_with_th_",true_th,"_n_",sample_size,"_p_",num_of_p,"_q_",num_of_q)
...
for(simulation_index in c(1:number_of_simulations)){
  ...
  for(p_regression in c(1:(num_of_p-1))){
    ...
    for(TAU_INDEX in c(1:9)){
      ...
      print_satement = paste0("Dataset-",simulation_index,". Starting QR at node ",p_regression,", quantile level ",tau_index,"\n")
      cat(print_satement)
      ...
      temp = as.matrix(temp);
      QR_P_sim = temp[,p_regression];
      Y = QR_P_sim
      q=ncol(rawU)
      ############################################################
      QR_Simple_P_sim =cbind(rep(1,sample_size), temp[,(p_regression+1):(num_of_p)]);
      ...
      v = array(0, dim = c(sample_size, num_of_p - p_regression +1, post_size ))
      v_Q = array(0, dim = c(num_of_q,num_of_p - p_regression,  post_size ))
      ##############################################################
      ...
      file_name = paste0("data_with_th_",true_th,"_n_",sample_size,"_p_",num_of_p,"_q_",num_of_q,"/True_betas/True_beta_matrix_n",sample_size,"_p_",num_of_p,"_q_",num_of_q,"_p_reg_",p_regression,"_tau_",tau_index_save,"_simu_",simulation_index,".csv")
      True_beta_matrix = read.csv(file_name, header = FALSE)
      est_beta_modified = hs_beta_est[,-c(1)]
      FROBENIOUS_norm_beta = norm(as.matrix(True_beta_matrix) - est_beta_modified, "F")
      
      #########################################
      file_name = paste0("data_with_th_",true_th,"_n_",sample_size,"_p_",num_of_p,"_q_",num_of_q,"/True_thetas/True_theta_matrix_n",sample_size,"_p_",num_of_p,"_q_",num_of_q,"_p_reg_",p_regression,"_tau_",tau_index_save,"_simu_",simulation_index,".csv")
      True_theta_matrix = read.csv(file_name, header = FALSE)
      est_theta_modified = hs_theta_est[,-c(1)]
      FROBENIOUS_norm_theta = norm(as.matrix(True_theta_matrix) - est_theta_modified, "F")
      #########################################
      ...
      temp = read.csv(filename_output, header = FALSE);
      QR_P_sim_TRUEs = temp[,((p_regression-1)*9+1):(p_regression*9)]
      ...
      temp_boolean_matrix = matrix(FALSE, nrow = num_of_q, ncol = p-1)
      
      TP_ts_this_reg_mod  = adjusted_true_cords[[p_regression]] - 1
      
      for(bool_col in 1:length(TP_ts_this_reg_mod))
      {
        temp_G_array = True_G_List$which.Xs.TP[[p_regression]][[1]][[bool_col]][[1]]
      ...
      file_name = paste0("HS_Results_with_th_",true_th,"_n_",sample_size,"_p_",num_of_p,"_q_",num_of_q,"/Post_process_MC_p_reg_",p_regression,
