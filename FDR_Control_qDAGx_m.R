general_v = array(0,c(sample_size,num_of_p,post_size))
th_seq = seq(0.01,1,0.01)

for(iter in 1:post_size)
{
  general_v[,misspecified_order[(p_regression+1):num_of_p],iter] = v[,2:ncol(rawX),iter]
}
general_v = general_v[,-misspecified_p_regression,]
############################################################################################
if(sum(True_beta_matrix == 0) !=0 && sum(True_beta_matrix !=0) !=0)
{
  ...
  for (iter in 1:post_size)
  {
    T_selec_3k_iters_1[,,iter] = (general_v[,,iter] !=0) &  (True_beta_matrix !=0)
    T_selec_3k_iters_2[,,iter] = (general_v[,,iter] !=0) &  (True_beta_matrix ==0)
  }
  ...
  for (iter in 1:post_size)
  {
    T_selec_no_TP_FP[,,iter] = (general_v[,,iter] !=0)
  }
...
}else if(sum(True_beta_matrix == 0) ==0 && sum(True_beta_matrix !=0) !=0 )
{
  #######################
  T_selec_3k_iters = array(0, c(sample_size, num_of_p - 1, post_size))
  for (iter in 1:post_size)
  {
    T_selec_3k_iters[,,iter] = (general_v[,,iter] !=0) &  (True_beta_matrix !=0)
  }
...
}else if(sum(True_beta_matrix == 0) !=0 && sum(True_beta_matrix !=0) ==0)
{
  ...
  T_post_prob_selec = apply(T_selec_3k_iters, c(1,2), mean)
  FPR_P = sum((T_post_prob_selec > 0.5) & (True_beta_matrix==0))/sum(True_beta_matrix==0)
  ########################
  TPR_P = 0
  AUC_P = 0
}else{
...
############################################################################################
general_v_Q = array(0,c(num_of_q, num_of_p, post_size))

for(iter in 1:post_size)
{
  general_v_Q[,misspecified_order[(p_regression+1):num_of_p],iter] = v_Q[,,iter]
}
general_v_Q = general_v_Q[,-misspecified_p_regression,]

if(sum(temp_boolean_matrix==0)!=0 && sum(temp_boolean_matrix!=0)!=0)
{
  T_selec_3k_iters_1 = array(0, c(num_of_q, num_of_p - 1, post_size))
  T_selec_3k_iters_2 = array(0, c(num_of_q, num_of_p - 1, post_size))
  th_seq = seq(0.01,1,0.01)
  TP_for_var_th = rep(0, length(th_seq))
  FP_for_var_th = rep(0, length(th_seq))
  ...
  for(th_iter in 1:length(th_seq))
  {
    for (iter in 1:post_size)
    {
      T_selec_3k_iters_1[,,iter] = (general_v_Q[,,iter] > th_seq[th_iter]) &  (temp_boolean_matrix !=0)
      T_selec_3k_iters_2[,,iter] = (general_v_Q[,,iter] > th_seq[th_iter]) &  (temp_boolean_matrix ==0)
    }
  ...
  #######################
  T_selec_3k_iters = array(0, c(num_of_q, num_of_p - 1, post_size))
  for (iter in 1:post_size)
  {
    T_selec_3k_iters[,,iter] = (general_v_Q[,,iter] >desired_th_for_G) &  (temp_boolean_matrix !=0)
  }
  T_post_prob_selec = apply(T_selec_3k_iters, c(1,2), mean)
  ...
  ########################
  T_selec_no_TP_FP = array(0, c(num_of_q, num_of_p - 1,  post_size))
  for (iter in 1:post_size)
  {
    T_selec_no_TP_FP[,,iter] = (general_v_Q[,,iter] > desired_th_for_G)
  }
  T_post_prob_selec_no_TP_FP = apply(T_selec_no_TP_FP, c(1,2), mean)
  #############
  threshold_seq = seq(0,1,1/1000);
...
}else if(sum(temp_boolean_matrix==0)==0 && sum(temp_boolean_matrix!=0)!=0)
{
  T_selec_3k_iters = array(0, c(num_of_q, num_of_p - 1, post_size))
  for (iter in 1:post_size)
  {
    T_selec_3k_iters[,,iter] = (general_v_Q[,,iter] >0.5) &  (temp_boolean_matrix !=0)
  }
  T_post_prob_selec = apply(T_selec_3k_iters, c(1,2), mean)
...
}else if(sum(temp_boolean_matrix==0)!=0 && sum(temp_boolean_matrix!=0)==0)
{
  ...
  T_post_prob_selec = apply(T_selec_3k_iters, c(1,2), mean)
  FPR_Q = sum((T_post_prob_selec > 0.5) & (temp_boolean_matrix==0))/sum(temp_boolean_matrix==0)
  TPR_Q = 0
  AUC_Q = 0
}else{
  TPR_Q = 0
